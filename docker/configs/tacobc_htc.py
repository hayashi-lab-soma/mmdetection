...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=(1074, 942), keep_ratio=False),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='RandomFlip', flip_ratio=0.8),
            #dict(type='RandomFlip', flip_ratio=0.5), #original
            dict(
                type='Normalize',
                mean=[123.675, 116.28, 103.53],
                std=[58.395, 57.12, 57.375],
...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        classes = classes,
        ann_file='TACOBCAWS_TRAIN.json',
        img_prefix='TACOBCAWS_TRAIN/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True, with_mask=True),
            dict(type='Resize', img_scale=(1074, 942), keep_ratio=False),
    ...
    val=dict(
        type='CocoDataset',
        classes = classes,
        ann_file='VAL_TACO163.json',
        img_prefix='VAL_TACO163/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
    ...
    test=dict(
        type='CocoDataset',
        classes = classes,
        ann_file='hokuto_test50.json',
        img_prefix='hokuto_test50/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
...
model = dict(
    ...
    train_cfg = dict(
        ...
        rpn_proposal=dict(
            ...
            # max_num=2000,
            # # nms_thr=0.7,
            # nms=dict(type='nms', iou_threshold=0.7),#changed
            # min_bbox_size=0),
    ...
    test_cfg = dict(
        ...
        rcnn=dict(
            ...
            max_per_img=100,
            mask_thr_binary=0.5)))
work_dir = './work_dir/jst/tacobc_htc_lr0.01'
